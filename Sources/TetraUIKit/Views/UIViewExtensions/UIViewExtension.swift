...
public extension NSView {
  ...
  /// Create a view with specified [children]. Children will be tagged with the order within the array.
  convenience init(
    @TetraUIViewBuilder children: () -> [NSView],
    withInsets insets: NSEdgeInsets? = nil,
    useSafeArea: Bool = false
  ) {
    self.init()
    subviews(addedWithInsets: insets, useSafeArea: useSafeArea, views: children)
  ...
  /// Load a view from nib by [name] in [bundle].
  static func loadedFrom<T: NSView>(nibNamed name: String, bundle : Bundle? = nil) -> T? {
    let nib = NSNib(nibNamed: name, bundle: bundle)
    return nib?.instantiate(withOwner: nil, topLevelObjects: nil) as? T
  }
  ...
  /// Set the [priority] for content compression resistance in [axis] for this view.
  @discardableResult func contentCompressionResistancePriority(
    setTo priority: NSLayoutConstraint.Priority,
    udpateWith priorityPublihser: AnyPublisher<NSLayoutConstraint.Priority, Never>? = nil,
    forAxis axis: NSLayoutConstraint.Orientation
  ) -> Self {
    self.setContentCompressionResistancePriority(priority, for: axis)
    if let view = self as? TetraUIViewCancellable {
      priorityPublihser?.sink(receiveValue: { [weak self] priority in
  ...
  /// Add shadow to the this view.
  @discardableResult func shadow(
    setToColor color: NSColor,
    updateWith colorPublisher: AnyPublisher<NSColor, Never>? = nil,
    offset: CGSize,
    udpateWith offsetPublisher: AnyPublisher<CGSize, Never>? = nil,
    radius: Double,
    updateWith radiusPublisher: AnyPublisher<Double, Never>? = nil,
    ...
    if let view = self as? TetraUIViewCancellable {
      colorPublisher?.sink(receiveValue: { [weak layer] color in
        layer?.shadowColor = color.cgColor
      }).store(in: &view.viewCancellables)

      offsetPublisher?.sink(receiveValue: { [weak layer] offset in
        layer?.shadowOffset = offset
      }).store(in: &view.viewCancellables)

      radiusPublisher?.sink(receiveValue: { [weak layer] radius in
        layer?.shadowRadius = radius
      }).store(in: &view.viewCancellables)

      opacityPublisher?.sink(receiveValue: { [weak layer] opacity in
        layer?.shadowOpacity = opacity
      }).store(in: &view.viewCancellables)
    }
  ...
  /// Add a gesture recognizer to the view.
  @discardableResult func gestureRecognizerAdded(_ recognizer: NSGestureRecognizer) -> Self {
    addGestureRecognizer(recognizer)
    return self
  }
  ...
  @discardableResult func subview(
    _ subview: NSView,
    addedWithInsets insets: NSEdgeInsets?,
    useSafeArea: Bool = false
  ) -> Self {
    self.subviews(addedWithInsets: insets, useSafeArea: useSafeArea) {
      subview
  ...
  @discardableResult func subviews(
    addedWithInsets insets: NSEdgeInsets?,
    useSafeArea: Bool = false,
    @TetraUIViewBuilder views: () -> [NSView]
  ) -> Self {
    let views = views()
    for view in views {
      addSubview(view)
  ...
  @discardableResult func subviews(
    insertedAtIndex index: Int,
    edWithInsets insets: NSEdgeInsets?,
    useSafeArea: Bool = false,
    @TetraUIViewBuilder views: () -> [NSView]
  ) -> Self {
    guard index <= subviews.count else { return self }
  ...
  func firstChildOfType<T: NSView>(_ type: T.Type) -> T? {
    if let child = subviews.first(where: { $0 is T }) {
      return child as? T
    } else {
      for subview in subviews {
